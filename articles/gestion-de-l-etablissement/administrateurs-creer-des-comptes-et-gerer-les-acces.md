---
title: "Administrateurs : créer des comptes et gérer les accès"
description: "Créer le compte d'un collègue, comprendre les cinq rôles, bloquer un accès et traiter les demandes de rattachement et de changement de rôle."
state: draft
collections: [gestion-de-l-etablissement]
intercom_us_id: "13635254"
intercom_us_url: ""
intercom_us_updated_at: 1770302971
---
## Objectif

Donner accès à Bacastages aux personnes de votre établissement, et tenir ces accès à jour.

Cet article s'adresse aux **administrateurs d'établissement** et aux **DDF**.

> **Un compte n'est pas une fiche professeur.** L'écran le dit en toutes lettres : *« Les fiches, et leur lien éventuel avec un compte, se gèrent depuis Mes professeur(e)s. »* Ici, on n'administre que des accès.

## Ce qu'il vous faut

- Un compte **Administrateur établissement** ou **DDF, ATDDF, BDE, Personnel de direction**
- Le nom, le prénom, l'e-mail et le téléphone de la personne

---

## 1. Ouvrir l'espace Établissement

Cliquez sur **« Établissement »** dans la barre de gauche.

## 2. Ouvrir la carte « Gestion des comptes »

L'écran s'intitule **« Gestion des comptes »** : *« Les accès à Bacastages : rôles, blocages et demandes de rattachement. »*

Il compte jusqu'à quatre onglets : **Utilisateurs**, **Demandes de rattachement**, **Demandes de rôle**, **Demandes d'école**. Une pastille rouge signale ce qui attend une décision.

---

## Créer le compte d'un collègue

## 3. Cliquer sur « Ajouter un utilisateur »

Le bouton se trouve en haut à droite.

> **Ce n'est pas une invitation.** Vous créez le compte vous-même, **mot de passe compris**, et vous le communiquez à la personne. Elle n'a pas d'e-mail à attendre.

## 4. Renseigner l'identité

**Prénom**, **Nom**, **Email**, **Téléphone**.

## 5. Choisir un mot de passe

**8 caractères au minimum**, avec une minuscule, une majuscule, un chiffre et un caractère spécial.

> **Dites à votre collègue de le changer à sa première connexion**, depuis l'onglet « Sécurité » de son compte. Un mot de passe que vous connaissez n'est plus le sien.

## 6. Choisir le rôle

Quatre rôles vous sont proposés : voir ci-dessous.

## 7. Créer le compte

Cliquez sur **« Créer l'utilisateur »**.

---

## Les cinq rôles

| Rôle | Ce qu'il fait |
|---|---|
| **Administrateur établissement** | Tout, y compris les accès et les paramètres |
| **Ddf, ATDDF, BDE, Personnel de direction** | Les offres, le suivi, les conventions, les filières, l'équipe |
| **Professeur(e) encadrant(e)** | Ses mini-stages, l'appel, ses comptes rendus |
| **Vie scolaire - Accueil** | Consulte les dossiers **et pointe les présences** |
| **Compte Inscriptions** | Inscrit des élèves dans les mini-stages d'autres établissements |

> **« Vie scolaire - Accueil » n'est pas un rôle en lecture seule.** Il pointe les présences : c'est sa raison d'être : la personne qui accueille les élèves à la grille est celle qui sait qui est arrivé.

> **Vous ne pouvez pas créer d'administrateur.** La fenêtre de création n'en propose pas : seuls professeur, DDF, Vie scolaire et Compte Inscriptions y figurent. Pour un second administrateur, écrivez au support.

---

## Bloquer un accès

Sur la ligne de la personne, le bouton **« Bloquer »** suspend son accès. Il devient **« Débloquer »** ensuite.

> **Bloquer n'est pas supprimer.** Le compte existe toujours, avec son historique ; il ne peut simplement plus entrer. C'est le geste à faire pour un départ en cours d'année.

---

## Traiter les demandes

Trois onglets, trois files :

- **Demandes de rattachement** : une personne a demandé à rejoindre votre établissement. Son compte existe déjà, elle attend votre accord.
- **Demandes de rôle** : un utilisateur de votre établissement demande à changer de rôle.
- **Demandes d'école** : les demandes liées à l'établissement lui-même.

> La pastille rouge de chaque onglet compte ce qui reste à trancher. L'entrée **« Établissement »** de la barre de gauche en porte une aussi : c'est souvent par là que vous apprendrez qu'une demande est arrivée.

---

## Besoin d'aide ?

- **Par email :** [support@bacastages.fr](mailto:support@bacastages.fr)
- **Par le chat :** la bulle en bas à droite de votre écran
